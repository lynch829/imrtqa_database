...
data = cell2mat(db.queryColumns('delta4', 'abspassrate', 'delta4', ...
...
% Plot dose pass rate vs gamma pass rate
subplot(2,2,2);
...
try
    m = fitlm(data(:,1), data(:,2), 'linear', 'RobustOpts', ...
...
rows{1,5} = sprintf('%0.3f', m.Coefficients{1,1});
rows{1,6} = sprintf('%0.3f', m.Coefficients{1,2});
rows{1,7} = sprintf('%0.3f', m.Coefficients{1,3});
rows{1,8} = sprintf('%0.3f', m.Coefficients{1,4});
rows{1,9} = sprintf('[%0.3f, %0.3f]', ci(1,:));
